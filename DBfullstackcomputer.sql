...
CREATE TABLE GIOHANG (
    ...
    -- Khóa ngoại liên kết với bảng bạn vừa tạo
    FOREIGN KEY (user_id) REFERENCES NGUOIDUNG(user_id),
    FOREIGN KEY (product_id) REFERENCES SANPHAM(product_id),
    
    -- Đảm bảo mỗi user chỉ có 1 dòng cho mỗi sản phẩm (Tránh trùng lặp)
...
select * from HINHANH_SP
select * from SANPHAM where product_id = 1017
select * from DANHMUC
select * from GIOHANG
select * from NGUOIDUNG
delete from GIOHANG where user_id=3
delete from SANPHAM where product_id = 1014
delete from HINHANH_SP where product_id = 1015
...
-- Chuỗi này tương ứng với mật khẩu '123' sau khi băm bằng Bcrypt
UPDATE NGUOIDUNG 
SET password = '$2b$12$R9h/cIPz0gi.URQHeS.Pz.S0yS8p.O4T88GZp.S.WfO5p8O5P5O5.' 
WHERE username = 'admin_thanh';

ALTER TABLE NGUOIDUNG ADD full_name NVARCHAR(100);
GO
UPDATE NGUOIDUNG 
SET password = '123' 
WHERE username = 'khachhang01';
GO